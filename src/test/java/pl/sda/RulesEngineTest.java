...
class RulesEngineTest {
    ...
    void isCreditApprovedYES() {
        Debtor debtor = new Debtor(20,10000,new int[] {1000,500},false,10000,2);
        Assertions.assertTrue(rulesEngine.isCreditApproved(debtor));
    }

    @Test
    void isCreditApprovedNO_wrongAge() {
        Debtor debtor = new Debtor(70,10000,new int[] {1000,500},false,10000,2);
        Assertions.assertFalse(rulesEngine.isCreditApproved(debtor));
    }

    @Test
    void isCreditApprovedNO_wrongIncome() {
        Debtor debtor = new Debtor(20,10,new int[] {1000,500},false,10000,2);
        Assertions.assertFalse(rulesEngine.isCreditApproved(debtor));
    }

    @Test
    void isCreditApprovedNO_wrongLoanInstallments() {
        Debtor debtor = new Debtor(20,10000,new int[] {1000,500,10000},false,10000,2);
        Assertions.assertFalse(rulesEngine.isCreditApproved(debtor));
    }

    @Test
    void isCreditApprovedNO_wrongRepaymentDelayed() {
        Debtor debtor = new Debtor(20,10000,new int[] {1000,500},true,10000,2);
        Assertions.assertFalse(rulesEngine.isCreditApproved(debtor));
    }

    @Test
    void isCreditApprovedNO_wrongAmountOfCredit() {
        Debtor debtor = new Debtor(20,10000,new int[] {1000,500},false,10000000,2);
        Assertions.assertFalse(rulesEngine.isCreditApproved(debtor));
    }

    @Test
    void isCreditApprovedNO_wrongPeriodInYears() {
        Debtor debtor = new Debtor(20,10000,new int[] {1000,500},false,10000,80);
        Assertions.assertFalse(rulesEngine.isCreditApproved(debtor));
    }
